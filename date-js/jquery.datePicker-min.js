...
Date.fullYearStart = "20"; (function() {
		...
		renderCalendar: function(t) {
			...
			if (t.showHeader != d.dpConst.SHOW_HEADER_NONE) {
				...
				for (var x = Date.firstDayOfWeek; x < Date.firstDayOfWeek + 7; x++) {
					...
					}).html(t.showHeader == d.dpConst.SHOW_HEADER_SHORT ? v.substr(0, 3) : v))
				}
			}
			...
			while (n++<q) {
				...
				for (var x = 0; x < 7; x++) {
					...
					//console.log("t2",t);
					//console.log("m",m);
					//console.log("m.getTime()",m.getTime());
					console.log(t.markerDate);
					console.log($.inArray(m.getTime(),t.markerDate));
					///////////////////////////////////////////
					...
					var y = d(C("td")).text(m.getDate() + "").addClass((j ? "current-month ": "other-month ") + (m.isWeekend() ? "weekend ": "weekday ") + ($.inArray(m.getTime(),t.markerDate)==-1 ? " ": "today")).data("datePickerDate", m.asString()).hover(A(k), g);
					u.append(y);
					if (t.renderCallback) {
	...
	d.extend(a.prototype, {
		...
		setStartDate: function(e) {
			...
			if (!this.startDate) {
				this.startDate = (new Date("12/31/999")).zeroTime()
			}
		...
		display: function(e) {
			...
			n.append(d("<div></div>").attr(o).css(j).append(d("<h2></h2>"), d('<div class="dp-nav-prev"></div>').append(d().bind("click",
			function() {
			...
			})), d('<div class="dp-nav-next"></div>').append(d().bind("click",
			function() {
				return m._displayNewMonth.call(m, this, 0, 1)
